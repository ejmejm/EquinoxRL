...
class EnvParams(environment.EnvParams):
    speed: float = 0.1 # How fast the agent moves
    turn_speed: float = 0.1 # How fast the agent turns
    agent_radius: float = 0.2 # Radius of the agent
    max_steps_in_episode: int = 100 # Steps in an episode

    # List of rooms, each room is given by a coordinate pair indicating (x, y) bottom left and (x, y) top right coordinates
    rooms: List[Tuple[float, float], Tuple[float, float]] = [((-2, 2), (2, 2))] # List of rooms
    spawn_point: Tuple[float, float] = (0, 0) # Spawn point of the agent


class RoombaEnv(environment.Environment[EnvState, EnvParams]):
    @property
